...
class TracyDebugger extends \ProcessWire\WireData implements \ProcessWire\Module, \ProcessWire\ConfigurableModule {
    ...
    public static function getModuleInfo() {
        return array(
            ...
            'summary' => \ProcessWire\__('Tracy debugger from Nette with several PW specific custom tools.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php'),
            'author' => 'Adrian Jones',
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/tracy-'.$tracyVersion.'/src/tracy.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/CodeProcessor.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
        ...
        if(static::$allowedTracyUser === 'development') {
            ...
            if(in_array('moduleDisabler', static::$showPanels) && $this->wire('config')->debug && $this->wire('config')->advanced) {
                ...
                if($this->wire('input')->cookie->tracyModulesDisabled) {
                    if (!file_exists($this->tracyCacheDir)) \ProcessWire\wiremkdir($this->tracyCacheDir);
        ...
        $this->wire()->addHookAfter('Wire::trackException', function($event) {
            ...
            if($this->wire('config')->ajax && ($exception instanceof \ProcessWire\WireException || $exception instanceof \ProcessWire\WireException)) {
                // intentionally blank
            }
            else {
                throw $exception;
 ...
 require_once(\ProcessWire\wire('files')->compile($consoleCodeFile,array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
    ...
    */
    ...
    public static function createEditorLink($file, $line, $linkText, $title = null) {
        ...
        elseif(!static::$isLocal && self::getDataValue('localRootPath') != '') $file = str_replace(\ProcessWire\wire('config')->paths->root, self::getDataValue('localRootPath'), $file);
        return '<a '.($title ? 'title="'.$title.'"' : '').' href="'.$file.'">'.$linkText.'</a>';
    }


    public static function removeCompilerFromPath($path) {
        $compilerCachePath = isset(\ProcessWire\wire('config')->fileCompilerOptions['cachePath']) ? \ProcessWire\wire('config')->fileCompilerOptions['cachePath'] : \ProcessWire\wire('config')->paths->cache . 'FileCompiler/';
 ...
 require_once(\ProcessWire\wire('files')->compile(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger' . '/includes/PwApiData.php',array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
    ...
    public function getModuleConfigInputfields(array $data) {
        ...
        $f->description = \ProcessWire\__('Maps editor links from live site to local dev files. Only used if you are viewing a live site, otherwise it is ignored.', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
        ...
        $f->notes = \ProcessWire\__('Default: 10', \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/TracyDebugger.module.php');
