...
class Debugger
{
 ...
 require_once(\ProcessWire\wire('files')->compile(dirname(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.7.x/src/Tracy/Debugger') . "/$path.php",array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
		}
		...
		if (self::$productionMode || connection_aborted()) {
			...
			if (!$firstTime) {
				...
				(function ($logged) use ($exception) {
					require self::$errorTemplate ?: \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.7.x/src/Tracy/Debugger' . '/assets/error.500.phtml';
				})(empty($e));
			} elseif (PHP_SAPI === 'cli') {
