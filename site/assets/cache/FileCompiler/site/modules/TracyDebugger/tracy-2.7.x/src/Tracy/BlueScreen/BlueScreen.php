...
class BlueScreen
{
	...
	{
		$this->collapsePaths[] = preg_match('#(.+/vendor)/tracy/tracy/src/Tracy/BlueScreen$#', strtr(\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.7.x/src/Tracy/BlueScreen', '\\', '/'), $m)
			? $m[1]
			: \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.7.x/src/Tracy/BlueScreen';
	}
		...
		if (Helpers::isAjax() && session_status() === PHP_SESSION_ACTIVE) {
			$_SESSION['_tracy']['bluescreen'][$_SERVER['HTTP_X_TRACY_AJAX']] = [
				'content' => Helpers::capture(function () use ($exception) {
					$this->renderTemplate($exception, \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.7.x/src/Tracy/BlueScreen' . '/assets/content.phtml');
				}),
				'time' => time(),
			];
			...
			$this->renderTemplate($exception, \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.7.x/src/Tracy/BlueScreen' . '/assets/page.phtml');
		}
	}
	...
	public function renderToFile(\Throwable $exception, string $file): bool
	{
		if ($handle = @fopen($file, 'x')) {
			...
			$this->renderTemplate($exception, \ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.7.x/src/Tracy/BlueScreen' . '/assets/page.phtml', false);
			ob_end_flush();
			ob_end_clean();
			fclose($handle);
			return true;
	...
	private function renderTemplate(\Throwable $exception, string $template, $toScreen = true): void
	{
		...
		$css = array_map('file_get_contents', array_merge([
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.7.x/src/Tracy/BlueScreen' . '/assets/bluescreen.css',
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.7.x/src/Tracy/BlueScreen' . '/../Toggle/toggle.css',
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.7.x/src/Tracy/BlueScreen' . '/../TableSort/table-sort.css',
			\ProcessWire\wire("config")->paths->root . 'site/modules/TracyDebugger/tracy-2.7.x/src/Tracy/BlueScreen' . '/../Dumper/assets/dumper.css',
